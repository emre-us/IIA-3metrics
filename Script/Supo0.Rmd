...
#### 1.1. OLS Estimator in Multiple Regression
...
#### 1.2. How Well Does the Regression Line Describe the Data?
...
##### 1.2.2. <ins>Standard error of the regression</ins>
...
##### <ins>Assumption 1</ins>: $\mathbb{E}(u_i | \vec{X}_i)=0$
...
##### <ins>Assumption 2</ins>: $(\vec{X}_i,Y_i)$ are i.i.d. draws from their joint distribution
...
##### <ins>Assumption 3</ins>: $\vec{X}_i$ and $u_i$ have nonzero finite fourth moment
...
##### <ins>Assumption 4</ins>: $\mathbf{X}$ has full column rank (i.e. no perfect multicollinearity)
...
##### <ins>Assumption 5</ins>: Homoskedasticity $Var(u_i|\vec{X}_i)=\sigma_u^2$
...
##### <ins>Assumption 6</ins>: Conditional distribution of $u_i$ given $\vec{X}_i$ is normal (normal errors)
...
#### 2.1. <ins>U</ins>nbiased <ins>E</ins>stimator
...
The sample mean is not the only unbiased estimator. For example $X_1$, the first observation, satisfies $\mathbb{E}(X_1)=\mu$, so it is also unbiased. In general, any unbiased weighted average $\dfrac{\sum_{i=1}^n\omega_iX_i}{\sum_{i=1}^n\omega_i}$ with $\omega_i \geq 0$ is unbiased for $\mu.$
...
#### 2.2. <ins>L</ins>inear conditionally <ins>U</ins>nbiased <ins>E</ins>stimators
...
\[
\widehat{\beta}_1 = \sum_{i=1}^n\omega_iY_i
\]
where the weights $\omega_1,\dots,\omega_n$ can depend on $X_1,\dots,X_n$ but not on $Y_1,\dots,Y_n.$
...
&= \sum_{i=1}^n\widehat{\omega}_iY_i \ \ \ \ \text{where } \widehat{\omega}_i=\frac{(X_i-\bar{X})}{\displaystyle\sum_{j=1}^n(X_j-\bar{X})^2}.
\end{align*}
\]
Since the weights $\widehat{\omega}_i, \ i=1,\dots,n$ in this expression for $\widehat{\beta}_1$ depend on $X_1,\dots,X_n$ but not on $Y_1,\dots,Y_n$, the OLS estimator $\widehat{\beta}_1$ is a linear estimator. Since it is also conditionally unbiased, it is linear conditionally unbiased estimator.
...
#### 2.3. <ins>B</ins>est <ins>E</ins>stimators
