...
In terms of the notation, $Y$ denotes the dependent variable, or regressand, and $X$s denote the explanatory variables, or regressors, with $k^{th}$ regressor indicated by $X_k$. The subscript $i$ is used to indicate cross-sectional data, and $t$ time series data, and they denote the $i^{th}$ or $t^{th}$ observation or value, and $X_{ki}$ or $X_{kt}$ denote the $i^{th}$ or $t^{th}$ observation on variable $X_k$. 
...
The estimator $a$ that minimizes the sum of squared prediction mistakes is called the *least squares estimator*. To minimize this, we take its derivative and set it to 0:
\[
...
In other words, the prediction mistake for the $i^{th}$ observation is $Y_i-(\hat{\beta}_0+\hat{\beta}_1X_i)=Y_i-\hat{\beta}_0-\hat{\beta}_1X_i,$ and the sum of the square of these prediction mistakes over all $n$ observations is:
\[
\sum_{i=1}^n \hat{u}^2_i = \sum_{i=1}^n(Y_i-\hat{\beta}_0-\hat{\beta}_1X_i)^2.
\]

Notice that this is the extension of LS in estimating the mean, which is now applied to linear regression model. In fact, if there is no $\hat{\beta}_0$ then the two are identical except for the notation. 

As before, our goal is to minimize this expression which is a function of $\hat{\beta}_0$ and $\hat{\beta}_1$. As is the case with $\bar{X}$ being the unique estimator that minimizes $\sum_{i=1}^n(X_i-a)^2$, there is also a unique pair of estimators that minimize $\sum_{i=1}^n(Y_i-\hat{\beta}_0-\hat{\beta}_1X_i)^2$. The estimators $\hat{\beta}_0$ and $\hat{\beta}_1$ that minimize the intercept $\beta_0$ and slope $\beta_1$ are called **o**rdinary **l**east **s**quares (**OLS**) estimators of $\beta_0$ and $\beta_1$.
...
Rearranging the terms would give us

\[
\sum_{i=1}^nY_i = n\hat{\beta}_0 + \hat{\beta}_1\sum_{i=1}^nX_i \\
\text{and} \\[4pt]
\sum_{i=1}^nY_iX_i = \hat{\beta}_0\sum_{i=1}^nX_i + \hat{\beta}_1\sum_{i=1}^nX_i^2.
\]

These simultaneous equations are known as *normal equations*. Rearranging the terms and dividing by $n$ shows that the OLS estimators $\hat{\beta}_0$ and $\hat{\beta}_1$ need to satisfy the following two equations
\[
\begin{align*}
...
\text{and} \\[4pt]
\hat{\beta}_1
= \frac{\dfrac{1}{n}\displaystyle\sum_{i=1}^nX_iY_i-\bar{X}\bar{Y}}{\dfrac{1}{n}\displaystyle\sum_{i=1}^nX_i^2-\bar{X}^2} 
...
>#### *Decomposition of OLS Estimators* {.unnumbered}
>
In practice it would not be possible to decompose but the discussion below is useful in analyzing the theoretical properties of $\hat{\beta}_0$ and $\hat{\beta}_1$.
>
Notice that $\sum(X_i-\bar{X})(Y_i-\bar{Y})$ depends on the values of $Y$, and the values of $Y$ depend on the values of $u$. This means, if the values of the disturbance term had been different in the $n$ observations, we would have obtained different values of $Y$. Which would in turn give us a different values for $\sum(X_i-\bar{X})(Y_i-\bar{Y})$, and therefore for $\hat{\beta}_1$; thus $\hat{\beta}_1$ is random. We can therefore decompose $\hat{\beta}_1$ into its random and nonrandom components:
\[
\begin{align}
\hat{\beta}_1
&= \frac{\displaystyle\sum_{i=1}^n(X_i-\bar{X})(Y_i-\bar{Y})}{\displaystyle\sum_{i=1}^n(X_i-\bar{X})^2} \\[6pt]
&= \frac{\displaystyle\sum_{i=1}^n(X_i-\bar{X})\left((\beta_0+\beta_1X_i+u_i)-(\beta_0+\beta_1\bar{X}+\bar{u})\right)}{\displaystyle\sum_{i=1}^n(X_i-\bar{X})^2} \\[6pt]
&= \frac{\displaystyle\sum_{i=1}^n(X_i-\bar{X})\left(\beta_1(X_i-\bar{X})+u_i-\bar{u}\right)}{\displaystyle\sum_{i=1}^n(X_i-\bar{X})^2} \\[6pt]
&= \frac{\displaystyle\sum_{i=1}^n\beta_1(X_i-\bar{X})^2 + \displaystyle\sum_{i=1}^n(X_i - \bar{X})(u_i-\bar{u})}{\displaystyle\sum_{i=1}^n(X_i-\bar{X})^2} \\[6pt]
&= \beta_1 + \frac{\displaystyle\sum_{i=1}^n(X_i-\bar{X})(u_i-\bar{u})}{\displaystyle\sum_{i=1}^n(X_i-\bar{X})^2} \\[6pt]
&= \beta_1 + \frac{\displaystyle\sum_{i=1}^n(X_i-\bar{X})u_i - \bar{u}\displaystyle\sum_{i=1}^n(X_i-\bar{X})}{\displaystyle\sum_{i=1}^n(X_i-\bar{X})^2} \\[6pt]
&= \beta_1 + \frac{\displaystyle\sum_{i=1}^n(X_i-\bar{X})u_i - \bar{u}\left(\sum_{i=1}^nX_i-n\bar{X}\right)}{\displaystyle\sum_{i=1}^n(X_i-\bar{X})^2} \\[6pt]
&= \beta_1 + \frac{\displaystyle\sum_{i=1}^n(X_i-\bar{X})u_i - \bar{u}\left(n\bar{X}-n\bar{X})\right)}{\displaystyle\sum_{i=1}^n(X_i-\bar{X})^2} \\[6pt]
&= \beta_1 + \frac{\displaystyle\sum_{i=1}^n(X_i-\bar{X})u_i}{\displaystyle\sum_{i=1}^n(X_i-\bar{X})^2} \\[6pt]
&= \beta_1 + \sum_{i=1}^n\left(\frac{(X_i-\bar{X})}{\displaystyle\sum_{i=1}^n(X_i-\bar{X})^2}\right)u_i.
\end{align}
\]
Therefore, $\hat{\beta}_1$ has two components: a fixed one that is equal to the true value $\beta_1$, and a random one that depends on the values of the disturbance term in the sample. That is, $\hat{\beta}_1$ is equal to the true value $\beta_1$ plus a linear combination of the values of the disturbance term in all the observations in the sample.
>
At this point we need to provide a clarification on the notation. The numerator $(X_i - \bar{X})$ changes as $i$ changes and is different for each observation. However, the denominator is the sum of squared deviations *for the whole sample* and is not dependent on $i$. In other words, the denominator is $(X_1-\bar{X})^2 + \dots + (X_n-\bar{X})^2$ which is not dependent on $i$. To avoid confusion, we can use a different index, $j$, for the summation in the denominator so that the expression for $\hat{\beta}_1$ becomes:
\[
\hat{\beta}_1 = \beta_1 + \sum_{i=1}^n\left(\frac{(X_i-\bar{X})}{\displaystyle\sum_{j=1}^n(X_i-\bar{X})^2}\right)u_i.
\]
A similar approach would then yield the following expression for $\hat{\beta}_0$:
\[
\hat{\beta}_0 = \beta_0 + \sum_{i=1}^n\left( \frac{1}{n} - \left( \frac{(X_i-\bar{X})}{\displaystyle\sum_{j=1}^n(X_i-\bar{X})^2} \right)\bar{X}  \right)u_i.
\]
In practice it would not be possible to decompose $\hat{\beta}_0$ and $\hat{\beta}_1$ since $\beta_0$ and $\beta_1$ are unknown. However, these are useful in analyzing the theoretical properties of $\hat{\beta}_0$ and $\hat{\beta}_1$.
...
\[
\hat{\beta}_1 = \frac{\displaystyle\frac{1}{n-1}\displaystyle\sum_{i=1}^n(X_i-\bar{X})(Y_i-\bar{Y})}{\displaystyle\frac{1}{n-1}\displaystyle\sum_{i=1}^n(X_i-\bar{X})^2} = \frac{\hat{\sigma}_{XY}}{\hat{\sigma}_X^2}.
\]
where the sample variance $\hat{\sigma}^2 = (n-1)^{-1}\sum(Y_i-\bar{Y})^2.$ 

So, the OLS predicted values $\hat{Y}_i$ can be written in the following ways:
\[
\begin{align}
\hat{Y}_i 
&= \hat{\beta}_0 + \hat{\beta}_1X_i \ , \ \text{for} \ i=1,\dots,n \\[4pt]
&=\left(\bar{Y}-\hat{\beta}_1\bar{X}\right) + \hat{\beta}_1X_i \\[4pt]
&= \bar{Y} - \hat{\beta}_1\left(X_i - \bar{X}\right) \\[4pt]
&= \bar{Y} - \left( \beta_1 + \sum_{i=1}^n\left(\frac{(X_i-\bar{X})}{\displaystyle\sum_{j=1}^n(X_i-\bar{X})^2}\right)u_i \right)\left(X_i - \bar{X}\right) \\[4pt]
&= \bar{Y} - \left( \frac{\displaystyle\sum_{i=1}^n(X_i-\bar{X})(Y_i-\bar{Y})}{\displaystyle\sum_{i=1}^n(X_i-\bar{X})^2} \right)\left(X_i - \bar{X}\right) \\[4pt]
&=\bar{Y} - \left( \frac{\hat{\sigma}_{XY}}{\hat{\sigma}_X^2} \right)\left(X_i - \bar{X}\right).
\end{align}
\]
The difference between $Y_i$ and its predicted value $\hat{Y}_i$ is then the *residual* for the $i^{th}$ observation:
\[
\begin{align}
\hat{u}_i
&= Y_i-\hat{Y}_i \\[4pt]
&= Y_i - \hat{\beta}_0-\hat{\beta}_1X_i \\[4pt]
&= Y_i - \left(\bar{Y}-\frac{\hat{\sigma}_{XY}}{\hat{\sigma}_X^2}(X_i - \bar{X})\right) \\[4pt]
&= Y_i - \bar{Y} + \frac{\hat{\sigma}_{XY}}{\hat{\sigma}_X^2}(X_i - \bar{X})  \ , \ \text{for} \ i=1,\dots,n.
\end{align}
\]
...
These OLS estimators are *point estimators*, which means that given the sample, each estimator will provide a single, or point, value of the relevant population parameter. This is in contrast to *interval estimators* which provide a range of possible values.

$$\\[0.5in]$$

### Precision of OLS Estimators: Standard Errors

Given that the OLS estimates are a function of the sample data, they will change as the samples change. Accordingly, we need a measure of precision of the estimators $\hat{\beta}$s to see how reliable they are. This measure is called the *standard error* of the estimator, which is the standard deviation of the sampling distribution of the estimator. In turn, the sampling distribution of an estimator is a distribution of the set of values of the estimator obtained from all possible samples of the same size from a given population.

To derive the standard error of $\hat{\beta}$ we therefore need to first obtain their variances:
\[
\sigma_{\hat{\beta}_1}^2 = \mathbb{E}\left(\left(\hat{\beta}_1 - \mathbb{E}(\hat{\beta_1}\right)^2\right)
\]
since $\mathbb{E}(\hat{\beta}_1^2) = \beta_1$ (see the [unbiased estimator](#ue) discussion below), this expression becomes
\[
\sigma_{\hat{\beta}_1}^2 = \mathbb{E}\left((\hat{\beta}_1 - \beta_1)^2\right).
\]
We can substitute the expression we derived for $\hat{\beta}_1$ after decomposing it its random and nonrandom components:
\[
\begin{align}
\sigma_{\hat{\beta}_1}^2 
&= \mathbb{E}\left((\beta_1 + \sum_{i=1}^n\left(\underbrace{\frac{(X_i-\bar{X})}{\displaystyle\sum_{j=1}^n(X_i-\bar{X})^2}}_{a_i}\right)u_i - \beta_1)^2\right) \\[6pt]
&= \mathbb{E}\left( \left( \sum_{i=1}^na_iu_i \right)^2 \right) \\[6pt]
&= \mathbb{E}\left( \sum_{i=1}^n a_i^2 u_i^2 + 2\sum_{i=1}^n\sum_{h\neq i} a_i a_h u_i u_h \right) \\[6pt]
&= \sum_{i=1}^n a_i^2\mathbb{E}\left( u_i^2 \right) + 2\sum_{i=1}^n\sum_{h\neq i} a_i a_h \mathbb{E}\left( u_i u_h \right)
\end{align}
\]
Notice that $\mathbb{E}(u_i^2)=\sigma_u^2$ (see the [homoskedasticity](#assmptnskedasticity) assumption below), and that $\mathbb{E}(u_iu_h)=0$ for $i\neq j$ (see the [no autocorrelation](#assmptnnosercorr) assumption below), so this expression becomes:
\[
\begin{align}
\sigma_{\hat{\beta}_1}^2 
&= \sum_{i=1}^n a_i^2\sigma_u^2 \\[6pt]
&= \sigma_u^2 \sum_{i=1}^n a_i^2 \\[6pt]
&= \sigma_u^2 \sum_{i=1}^n \left( \frac{(X_i-\bar{X})}{\displaystyle\sum_{j=1}^n(X_i-\bar{X})^2} \right)^2 \\[6pt]
&= \sigma_u^2 \frac{\displaystyle\sum_{i=1}^n(X_i-\bar{X})^2}{\left( \displaystyle\sum_{j=1}^n(X_i-\bar{X})^2 \right)^2} \\[6pt]
&= \frac{\sigma_u^2}{\displaystyle\sum_{i=1}^n(X_i-\bar{X})^2}.
\end{align}
\]
A similar algebraic approach would then give the variance for $\hat{\beta}_0$ as:
\[
\sigma_{\hat{\beta}_0}^2 = \sigma_u^2 \frac{\displaystyle\sum_{i=1}^n X_i^2}{n\displaystyle\sum_{i=1}^n (X_i-\bar{X})^2} = \sigma_u^2 \left( \frac{1}{n} + \frac{\bar{X}^2}{\displaystyle\sum_{i=1}^n (X_i-\bar{X})^2} \right).
\]
Looking at the expressions for $\sigma_{\hat{\beta}_0}^2$ and $\sigma_{\hat{\beta}_1}^2$ we can see that they will be smaller, the larger $\sum(X_i-\bar{X})^2$ is. The sizes of $\sigma_{\hat{\beta}}^2$ s in turn depend both on the number of observations and on the size of the deviations of $X_i$ from its sample mean.

However, because $\sigma_u^2$ is unknown, we cannot calculate the population variances of either $\hat{\beta}_0$ or $\hat{\beta}_1$. In practice, therefore, we derive an estimator of $\sigma_u^2$ from the *residuals*. To derive this recall that $Y_i=\beta_0+\beta_1X_i+u_i$ and that $\bar{Y}=\beta_0+\beta_1\bar{X}+\bar{u}$. Subtracting the latter from the former gives us
\[
Y_i-\bar{Y}=\beta_1(X_i-\bar{X})+(u_i-\bar{u}).
\]
We can do the same for the estimates whereby we subtract $\bar{Y}=\hat{\beta}_0+\hat{\beta}_1\bar{X}$ from $Y_i=\hat{\beta}_0+\hat{\beta}_1X_i+\hat{u}_i$ and obtain $Y_i-\bar{Y}=\hat{\beta}_1(X_i-\bar{X})+\hat{u}_i,$ or $\hat{u}_i=Y_i-\bar{Y}-\hat{\beta}_1(X_i-\bar{X}).$ Putting these together, we get:
\[
\begin{align}
\hat{u}_i 
&= Y_i-\bar{Y} - \hat{\beta}_1(X_i-\bar{X}) \\
&= \beta_1(X_i-\bar{X}) + (u_i-\bar{u}) - \hat{\beta}_1(X_i-\bar{X})
\end{align}
\]
This is for observation $i$. We can square this and sum for all $i$:
\[
\sum_{i=1}^n\hat{u}_i^2 = (\hat{\beta}_1 - \beta_1)^2 \sum_{i=1}^n(X_i-\bar{X})^2 + \sum_{i=1}^n(u_i-\bar{u})^2 - 2(\hat{\beta}_1 - \beta_1)\sum_{i=1}^n(X_i-\bar{X})(u_i-\bar{u}).
\]
Taking expectations of both sides, we get:
\[
\mathbb{E}\left( \sum_{i=1}^n\hat{u}_i^2 \right) = \mathbb{E}\left( (\hat{\beta}_1 - \beta_1)^2 \right) \sum_{i=1}^n(X_i-\bar{X})^2 + \mathbb{E}\left( \sum_{i=1}^n(u_i-\bar{u})^2 \right) - 2\mathbb{E}\left( (\hat{\beta}_1 - \beta_1)\sum_{i=1}^n(X_i-\bar{X})(u_i-\bar{u}) \right).
\]
which, after some algebraic simplifications, becomes:
\[
\begin{align}
\mathbb{E}\left( \sum_{i=1}^n\hat{u}_i^2 \right) 
&= \sigma_u^2 + (n-1)\sigma_u^2 - 2\sigma_u^2 \\
&= (n-2)\sigma_u^2
\end{align}
\]
Thus, we can estimate $\sigma_u^2$ with the following:
\[
\hat{\sigma}_u^2 = \frac{\displaystyle\sum_{i=1}^n\hat{u}_i^2}{n-2}
\]
where $\hat{u}_i^2$ is the OLS estimator of the true but unknown $\sigma^2$.

We can now put all of these together to derive the standard errors:
\[
\begin{align}
se(\hat{\beta}_0) &= \sqrt{\hat{\sigma}_u^2 \frac{\displaystyle\sum_{i=1}^n X_i^2}{n\displaystyle\sum_{i=1}^n (X_i-\bar{X})^2}} = \sqrt{\hat{\sigma}_u^2 \left( \frac{1}{n} + \frac{\bar{X}^2}{\displaystyle\sum_{i=1}^n (X_i-\bar{X})^2} \right)} \\[10pt]
se(\hat{\beta}_1) &= \sqrt{\frac{\hat{\sigma}_u^2}{\displaystyle\sum_{i=1}^n(X_i-\bar{X})^2}}
\end{align}
\]
It is important to emphasize here that the standard error gives only a general guide to the likely accuracy of a regression coefficient. It enables us to obtain some idea of the width of its pdf but not whether our regression estimate comes from the middle of the function, and is therefore accurate, or from the tails, and is therefore relatively inaccurate.

The higher the variance of the disturbance term, $\sigma_u^2$, the higher the sample variance of the residuals, $\hat{\sigma}_u$ is likely to be, and hence the higher will be the standard errors of the coefficients in the regression equation, reflecting the risk that the coefficients are inaccurate. However, it is only a risk. It is possible that, in any particular sample, the effects of the disturbance term in the different observations will cancel each other out and the regression coefficients will be accurate after all. However, there is no way of telling whether that happens in any particular sample.
...
Lets take a pause and briefly recap what we have discussed so far. We considered the linear single regression model $Y_i=\beta_0+\beta_1X_i+u_i$ and said that this population regression function is not directly observable and that we need to estimate it from the sample regression function $Y_i = \hat{Y}_i + u_i$ where $\hat{Y}_i=\hat{\beta}_0+\hat{\beta}_1X_i$. Our goal is to ensure that the estimated conditional mean value $\hat{Y}_i$ is as close to the population $Y_i$ as possible given $n$ pair of observations on $Y$ and $X$. This means, minimizing the residuals, though, for some additional desirable statistical properties, we would like to minimize instead the sum of squared residuals:
\[
\sum_{i=1}^n \hat{u}^2_i = \sum_{i=1}^n(Y_i-\hat{\beta}_0-\hat{\beta}_1X_i)^2
\]
which is a function of $\hat{\beta}_0$ and $\hat{\beta}_1$. To minimize, we take their partial derivatives with respect to $\hat{\beta}_0$ and $\hat{\beta}_1$ and set them to 0. This yields:
\[
\begin{align}
\hat{\beta}_0
&= \bar{Y}-\hat{\beta}_1\bar{X}\\[4pt]
\text{and} \\[4pt]
\hat{\beta}_1 
&= \frac{\displaystyle\sum_{i=1}^n(X_i-\bar{X})(Y_i-\bar{Y})}{\displaystyle\sum_{i=1}^n(X_i-\bar{X})^2}.
\end{align}
\]
These expressions show that $\hat{\beta}_0$ and $\hat{\beta}_1$ are dependent on the values in the sample and thus random, and we additionally discussed in the box how these can be decomposed into random and nonrandom components.

We then showed that $\hat{\beta}_1$ can also be expressed as $\frac{\hat{\sigma}_{XY}}{\hat{\sigma}_X^2}$ which leads us to be able to write the two components of the regression $Y_i=\hat{Y}_i+\hat{u}_i$ in the following ways:
\[
\begin{align}
\hat{Y}_i 
...
&= \bar{Y} - \left( \beta_1 + \sum_{i=1}^n\left(\frac{(X_i-\bar{X})}{\displaystyle\sum_{j=1}^n(X_i-\bar{X})^2}\right)u_i \right)\left(X_i - \bar{X}\right) \\[4pt]
&= \bar{Y} - \left( \frac{\displaystyle\sum_{i=1}^n(X_i-\bar{X})(Y_i-\bar{Y})}{\displaystyle\sum_{i=1}^n(X_i-\bar{X})^2} \right)\left(X_i - \bar{X}\right) \\[4pt]
&=\bar{Y} - \left( \frac{\hat{\sigma}_{XY}}{\hat{\sigma}_X^2} \right)\left(X_i - \bar{X}\right)  \ , \ \text{for} \ i=1,\dots,n \\[8pt]
\text{and} \\[8pt]
\hat{u}_i
&= Y_i-\hat{Y}_i \\[4pt]
&= Y_i - \hat{\beta}_0-\hat{\beta}_1X_i \\[4pt]
...
\end{align}
\]
Since these OLS estimates are sample-dependent and will change as the samples change, we introduced a measure of precision called *standard error*, which is the squared root of the variance of each of these estimators, and expressed as:
\[
\begin{align}
se(\hat{\beta}_0) 
&= \sqrt{\hat{\sigma}_u^2 \frac{\displaystyle\sum_{i=1}^n X_i^2}{n\displaystyle\sum_{i=1}^n (X_i-\bar{X})^2}} = \sqrt{\hat{\sigma}_u^2 \left( \frac{1}{n} + \frac{\bar{X}^2}{\displaystyle\sum_{i=1}^n (X_i-\bar{X})^2} \right)} \\[10pt]
se(\hat{\beta}_1) 
&= \sqrt{\frac{\hat{\sigma}_u^2}{\displaystyle\sum_{i=1}^n(X_i-\bar{X})^2}} \\[10pt]
\text{where  } \hat{\sigma}_u^2 
&= \frac{\displaystyle\sum_{i=1}^n\hat{u}_i^2}{n-2}.
\end{align}
\]
The standard errors are also useful in establishing confidence intervals and in testing statistical hypotheses.
...
We can extend the above discussion to multiple regression model $Y_i=\beta_0+\beta_1X_{1i}+\beta_2X_{2i}+\dots+\beta_kX_{ki}+u_i$ for $i=1,\dots,n$ where, as before, $u$ is the stochastic disturbance term, $i$ is the $i^{th}$ observation which, in time series, would be $t$ to denote $t^{th}$ observation. 

The population regression line is the relationship that holds, on average, between $Y$ and the $X$s in the population:
\[
\mathbb{E}(Y_i | X_{1i}=x_1, X_{2i}=x_2, \dots, X_{ki}=x_k) = \beta_0 + \beta_1x_1 + \beta_2x_2 + \dots + \beta_{ki}x_k.
\]
This expression states the conditional mean or expected value of $Y$ conditional upon the given, or fixed, values of $X_{ji}, j=1,\dots,k$. This means, as in the single regression case, multiple regression analysis is conditional upon the *fixed values of the regressors*, and it yields the *average value of $Y$*, or the *mean response of $Y$* for the given values of the regressors.

The intercept $\beta_0$ is the expected value of $Y$ when all the $X$s are equal to 0. The intercept can also be thought of as the coefficient on a regressor $X_0 = 1$ for all $i$. The coefficients $\beta_1,\dots,\beta_k$ are known as *partial regression coefficients* or *partial slope coefficients*. The coefficient $\beta_1$ is the expected difference in $Y_i$ associated with a unit difference in $X_1$, holding other regressors $X_2,\dots,X_k$ constant. In other words, $\beta_1$ measures the *change* in $\mathbb{E}(Y)$, the mean value of $Y$, per unit change in $X_1$, holding the values of other $X$s constant. In other words, it gives the *direct effect* or *net effect* on the mean value of $Y$ in response to a change of one uni in $X_1$, net of any effect that other $X$s may have on the mean value of $Y$. The coefficients on the other $X$s are interpreted similarly.

As was the case with single regression, we need to estimate the population parameters. The sample regression function is
\[
\begin{align}
Y_i 
&= \hat{Y}_i + \hat{u}_i \\[4pt]
&= \hat{\beta}_0 + \hat{\beta}_1X_{1i} + \dots + \hat{\beta}_kX_{ki} + \hat{u}_i.
\end{align}
\]
Our goal is still to get $\hat{Y}_i$ as close to $Y_i$ as possible, which means minimizing the residual sum of squares (RSS) $\sum\hat{u}_i^2$. The RSS over n observations is
\[
RSS = \sum_{i=1}^n\hat{u}_i^2 = \sum_{i=1}^n(Y_i-\hat{\beta}_0-\hat{\beta}_1X_{1i}-\hat{\beta}_2X_{2i}-\dots-\hat{\beta}_kX_{ki})^2
\]
To minimize this expression, we take partial derivatives with respect to each of the coefficients and set it to 0:
\[
\begin{align}
\frac{\partial}{\partial \hat{\beta}_0}\sum_{i=1}^n\hat{u}_i^2 
&= -2\sum_{i=1}^n(Y_i - \hat{\beta}_0 - \hat{\beta}_1X_i - \dots - \hat{\beta}_kX_{ki}) = -2\sum_{i=1}^n\hat{u}^2_i = 0 \\[4pt]
\frac{\partial}{\partial \hat{\beta}_1}\sum_{i=1}^n\hat{u}_i^2 
&= -2\sum_{i=1}^n(Y_i - \hat{\beta}_0 - \hat{\beta}_1X_i - \dots - \hat{\beta}_kX_{ki})X_{1i} = -2\sum_{i=1}^n\hat{u}^2_iX_{1i} = 0 \\[4pt]
&\vdots \\[4pt]
\frac{\partial}{\partial \hat{\beta}_k}\sum_{i=1}^n\hat{u}_i^2 
&= -2\sum_{i=1}^n(Y_i - \hat{\beta}_0 - \hat{\beta}_1X_i - \dots - \hat{\beta}_kX_{ki})X_{ki} = -2\sum_{i=1}^n\hat{u}^2_iX_{ki} = 0
\end{align}
\]
These yield expressions that are extensions of the simple regression, which, can get unwieldy very quickly. For $k=2$ the expressions are:
\[
\begin{align}
\hat{\beta}_0
&= \bar{Y} - \hat{\beta}_1\bar{X}_1 - \hat{\beta}_2\bar{X_2} \\[4pt]
\text{and} \\[4pt]
\hat{\beta}_1
&= \frac{\displaystyle\sum_{i=1}^n(X_{1i}-\bar{X}_1)(Y_i-\bar{Y}) \displaystyle\sum_{i=1}^n(X_{2i} - \bar{X}_2)^2 - \displaystyle\sum_{i=1}^n(X_{2i}-\bar{X}_2)(Y_i - \bar{Y}) \displaystyle\sum_{i=1}^n(X_{1i}-\bar{X}_1)(X_{2i}-\bar{X}_2)}{\displaystyle\sum_{i=1}^n(X_{1i}-\bar{X}_1)^2 \displaystyle\sum_{i=1}^n(X_{2i}-\bar{X}_2)^2 - \left( \displaystyle\sum_{i=1}^n(X_{1i}-\bar{X}_1)(X_{2i}-\bar{X}_2)  \right)^2} \\[4pt]
\hat{\beta}_2
&= \frac{\displaystyle\sum_{i=1}^n(X_{2i}-\bar{X}_2)(Y_i-\bar{Y}) \displaystyle\sum_{i=1}^n(X_{1i} - \bar{X}_1)^2 - \displaystyle\sum_{i=1}^n(X_{1i}-\bar{X}_1)(Y_i - \bar{Y}) \displaystyle\sum_{i=1}^n(X_{2i}-\bar{X}_2)(X_{1i}-\bar{X}_1)}{\displaystyle\sum_{i=1}^n(X_{2i}-\bar{X}_2)^2 \displaystyle\sum_{i=1}^n(X_{1i}-\bar{X}_1)^2 - \left( \displaystyle\sum_{i=1}^n(X_{2i}-\bar{X}_2)(X_{1i}-\bar{1}_2)  \right)^2}. \\[4pt]
\end{align}
\]
These are presented here to show how quickly it can become unwieldy even for $k=2$. For multiple explanatory variables, it is better to use linear algebra instead.

<div style="background-color:rgba(237, 231, 225, 1); text-align:left; vertical-align: middle; padding:0px; margin-top:10px; margin-bottom:20px">

>
#### Multiple Regression in Linear Algebra
>
To express in matrix notation define the following vectors and matrices:
\[
\vec{Y} = \begin{pmatrix}
Y_1 \cr
Y_2 \cr
...
>
Using the residual vector we also have $\mathbf{X}^T\hat{\vec{u}}=0$ since $\sum X_i\hat{u}_i=0.$

</div>

Just as we did for the single regression, we can also derive the standard errors of the estimators. For $k=2$, we have
\[
\begin{align}
Var(\hat{\beta}_0) 
= \hat{\sigma}_{\hat{\beta}_0}
&= \hat{\sigma}_u^2 \left( \frac{1}{n} + \frac{\bar{X}_1^2 \displaystyle\sum_{i=1}^n(X_{2i}-\bar{X}_2)^2 + \bar{X}_2^2 \displaystyle\sum_{i=1}^n(X_{1i}-\bar{X}_1)^2 - 2\bar{X}_1\bar{X}_2\displaystyle\sum_{i=1}^n(X_{1i}-\bar{X}_1)(X_{2i}-\bar{X}_2)}{\displaystyle\sum_{i=1}^n(X_{1i}-\bar{X}_1)^2 \displaystyle\sum_{i=1}^n(X_{2i}-\bar{X}_2)^2 - \left( \displaystyle\sum_{i=1}^n(X_{1i}-\bar{X}_1)(X_{2i}-\bar{X}_2)  \right)^2}    \right) \\[10pt]
se(\hat{\beta}_0) 
&= \sqrt{Var(\hat{\beta}_0)} = \sqrt{\hat{\sigma}_{\hat{\beta}_0}} \\[10pt]
Var(\hat{\beta}_1) 
= \hat{\sigma}_{\hat{\beta}_1}
&= \hat{\sigma}_u^2 \frac{\displaystyle\sum_{i=1}^n(X_{2i}-\bar{X}_2)^2}{\left( \displaystyle\sum_{i=1}^n(X_{1i}-\bar{X}_1)^2  \right) \left( \displaystyle\sum_{i=1}^n(X_{2i}-\bar{X}_2)^2 \right) - \left( \displaystyle\sum_{i=1}^n(X_{1i}-\bar{X}_1)(X_{2i}-\bar{X}_2)  \right)^2} \\[10pt]
&= \frac{\hat{\sigma}_u^2}{\displaystyle\sum_{i=1}^n(X_{1i}-\bar{X}_1)(1-\rho_{1,2}^2)} \\[10pt]
se(\hat{\beta}_1) 
&= \sqrt{Var(\hat{\beta}_1)} = \sqrt{\hat{\sigma}_{\hat{\beta}_1}}
\end{align}
\]
where
\[
\begin{align}
\hat{\sigma}_u^2 
&= \frac{1}{n-k}\displaystyle\sum_{i=1}^n\hat{u}_i^2 \ , \ \\[6pt]
\displaystyle\sum_{i=1}^n\hat{u}_i^2 
&= \displaystyle\sum_{i=1}^n(Y_i - \bar{Y}) - \hat{\beta}_1\displaystyle\sum_{i=1}^n(Y_i - \bar{Y})(X_{1i}-\bar{X}_1) - \hat{\beta}_2\displaystyle\sum_{i=1}^n(Y_i-\bar{Y})(X_{2i}-\bar{X}_2) \ , \ \\[6pt]
\text{and the sample covariance between $X_1$ and $X_2$ is: } \\[6pt]
\rho_{1,2}^2 
&= \frac{\left( \displaystyle\sum_{i=1}^n(X_{1i}-\bar{X}_1)(X_{2i}-\bar{X}_2)  \right)^2}{\displaystyle\sum_{i=1}^n(X_{1i}-\bar{X}) \displaystyle\sum_{i=1}^n(X_{2i}-\bar{X})}.
\end{align}
\]
...
The discussion in this section is aimed to develop an understanding as to the extent the regressor accounts for the variation in $Y$ and the extent the observations cluster around the regression line. Different textbooks use slightly different terminology ranging from *Goodness of Fit* to *Measures of Fit* and *Prediction Accuracy* for this discussion.
...
OLS can be thought of as decomposing each $Y_i$ into two parts, a fitted value $\hat{Y}_i$ and a residual $\hat{u}_i$ whereby $Y_i = \hat{Y}_i - \hat{u}_i$. The *regression $R^2$*, which is sometimes also referred to as the *coefficient of determination*, is the ratio of the sample variance of $Y$ explained by $X$s. In other words, $R^2$ is the proportion of the variation in $Y$ explained by the regression:
\[
R^2 = \frac{ESS}{TSS} = \frac{\displaystyle\sum_{i=1}^n(\hat{Y}_i-\bar{Y})^2}{\displaystyle\sum_{i=1}^n(Y_i-\bar{Y})^2}
\]
where ESS is the explained sum of squares and TSS is the total sum of squares. 

The $R^2$ can also be expressed in terms of the fraction of variance of $Y_i$ that is *not explained* by $X_i$
\[
R^2 = 1 - \frac{RSS}{TSS} = \frac{\displaystyle\sum_{i=1}^n\hat{u}_i^2}{\displaystyle\sum_{i=1}^n(Y_i-\bar{Y})^2} = frac{\displaystyle\sum_{i=1}^n(Y_i - \hat{Y}_i)}{\displaystyle\sum_{i=1}^n(Y_i-\bar{Y})^2}.
\]
Also note that
\[
\begin{align*}
...
#### *Relationship between R-Squared and Variance of Partial Regression Coefficient* {.unnumbered}

When we derived the variance of $\hat{\beta}_1$ above we said
\[
Var(\hat{\beta}_1) = \frac{\hat{\sigma}_u^2}{\displaystyle\sum_{i=1}^n(X_{1i}-\bar{X}_1)(1-\rho_{1,2}^2)}.
\]
we can extend this to $k$ regressors with $R^2$:
\[
Var(\hat{\beta}_j) = \frac{\hat{\sigma}_u^2}{\displaystyle\sum_{i=1}^n(X_{ji}-\bar{X}_j)(1-R_j^2)}
\]
where $\hat{\beta}_j$ is the partial regression coefficient of regressor $X_j$, and $R_j^2$ is the $R^2$ in the regression of $X_j$ on the remaining regressors. This relationship is particularly useful when discussion multicollinearity. 
...
Sometimes also referred to as *standard error of the estimate*, or as *root mean squared error*, the standard error of the regression (SER) is an estimator of the standard deviation of the regression error $u_i$ Since regression errors $u_1,\dots,u_n$ are unobserved, the SER is computed using their sample counterparts, the OLS residuals $\hat{u}_1, \dots, \hat{u}_n.$ 
\[
SER = \hat{\sigma}_\hat{u} = \sqrt{\hat{\sigma}_\hat{u}^2}
\]
where
\[
s_\hat{u}^2 = \frac{1}{n-k-1}\sum_{i=1}^n\hat{u}_i^2 = \frac{RSS}{n-k-1}
\]
The estimate $\hat{\sigma}_\hat{u}$ is an estimate of the standard deviation in the unobservables affecting $Y$. That is, it estimates the standard deviation in $Y$ after the effect of $X$ has been taken out. 
...
For this, not only the functional form of the model, such as $Y_i=\beta_0+\beta_1X_i+u_i$ is needed, but also certain assumptions about the way in which $Y_i$ are generated. This is because $Y_i$ depends on both $X_i$ and $u_i$. So unless we are specific about how $X_i$ and $u_i$ are generated, we cannot make any statistical inference about $Y_i$ as well as the $\beta$s. Therefore, the assumptions made about $X_i$ and $u_i$ are critical for valid interpretation of regression estimates.

These assumptions below discuss the statistical properties of OLS that is estimating the parameters in an underlying population model. These statistical models have nothing to do with a particular sample, but rather with the property of estimators when random sampling is done repeatedly. There are two sets of assumptions we consider here. One set of assumptions are those about the explanatory variables and the other set is about the error terms.
...
**Extra**: In time series, the population regression model would be expressed as $Y_t = \beta_0 + \beta_1X_{1t} + \beta_2X_{2t} + \dots + \beta_kX_{kt} + u$ where $t$ in $X_{jt}$ denotes the time period and $j$ indicates one of the $k$ regressors. In this sense, we can let $\vec{X}_t^T = (X_{1t}, X_{2t}, \dots, X_{kt})$ denote the set of all independent variables in the equation at time $t$. Similarly, $\mathbf{X}$ can be thought of as an array with $n$ rows and $k$ columns, so the $t^{th}$ row of $\mathbf{X}$ is $\vec{X}_t^T$
...
### <ins>Assumption 2a</ins>: Fixed Regressors $X_1, X_2, \dots, X_n$ {#fixedregressor}

This effectively means that the $X_1, X_2, \dots, X_n$ are nonstochastic, i.e. its value is a fixed number. Since the regression analysis is conditional upon the given $X$ values $(X_1=x_1,\dots,X_n-x_n)$, we are assuming $X$s are nonstochastic. Also, $X$ being fixed means, by definition, it is uncorrelated with the disturbance term $u$.

This assumption is very restrictive and we cannot generalize the analysis to cases where there are different sets of sample values of $X$. We will relax this in [Assumption 2b](#asmptnrandsamp) where the sample values of the $X$ variables are drawn randomly from fixed populations instead.

The assumption of $X$s being fixed is helpful for three reasons. 
- it helps to simplify the analysis and develops an understanding of complexities of regression analysis gradually. 
- in experimental situations it may be somewhat realistic to assume that $X$ values are fixed. For example, one may be testing different levels of intervention on the same thing, such as varying levels of fertilizers in different parcels of the same land, to see its impact. If the experimenter uses the same level of $X$ to the $i^{th}$ parcel in all repetitions of the experiment, then the value of $X_i$ does not change from one sample to the next. However, note that, in modern protocols for experiments, level of $X$ would be assigned via computerized random number generator to avoid any experimenter's bias creeping in, which would mean that the level of $X$ is random and [Assumption 2b](#asmptnrandsamp) would hold (also see [exogeneity](#asmptnexogen) assumption). It may also be the case that sometimes $X$ is fixed for a specific purpose such as when trying to ascertain the average annual salary $(Y)$ with various levels of education $(X)$. In this case, $X$ can be considered fixed, or nonrandom.
- the statistical results of linear regression based on fixed $X$s are still valid even if the $X$ variables are random, if certain conditions are met, including the condition that regressor $X$ and the error term $u_i$ are independent.
...
### <ins>Assumption 2b</ins>: Random Sampling: $(X_i,Y_i)$ are i.i.d. draws from their joint distribution {#asmptnrandsamp .unnumbered}

In statistics the term *random sampling* has a special meaning in that $Y_1, Y_2, \dots, Y_n$ constitute a random sample size of $n$ if all these $Y$s are drawn independently from the same probability distribution. That is, each $Y_i$ has the same pdf. Sampling, or selection, from the population is also random, which means the values of the observations $Y_1, \dots, Y_n$ are also random since their values will differ if different members of the population are chosen. Thus, the act of random sampling means that $Y_1,\dots,Y_n$ can be treated as random variables. Before they are sampled they can take on many possible values, but after they are sampled we have a specific value for each observation $Y_1,\dots,Y_n$.
...
This assumption therefore states that if the observations are drawn by simple random sampling from a single large population, then $(X_i,Y_i), i=1,\dots ,n$ are i.i.d. 
...
where $u_i$ is the error term or disturbance for observation $i$. Notice this is not the residual $\hat{u}_i$ but disturbance $u_i$. For example, we can write a CEO salary equation for a particular CEO $i$ as
...
### <ins>Assumption 3</ins>: Sample variation in the regressor

This is a very weak assumption that most textbooks omit as not worth mentioning but some other textbooks provide it since this assumption is nevertheless needed. It says that the sample outcomes on $x$, namely ${x_i, i=1,\dots,n}$, are not all the same value. If all the sample values of x_i is the same, then sample variance of $x_i$ is zero, then we would not be able to compute $\hat{\beta}_1$ since both the numerator and denominator of
\[
\hat{\beta}_1 = \frac{\displaystyle\sum_{i=1}^n(X_i-\bar{X})(Y_i-\bar{Y})}{\displaystyle\sum_{i=1}^n(X_i-\bar{X})^2}
\]
would equal to 0, since $X_i$ would equal to $\bar{X}$ for all $i$.


$$\\[0.5in]$$


### <ins>Assumption 4</ins>: Large outliers are unlikely: $Y_i$ and $X_i$ have nonzero finite fourth moment {#asmptn4thmmnt .unnumbered}

This assumption is states that large outliers are unlikely. By stating that $0 < \mathbb{E}(X_i^4) < \infty$ and $0 < \mathbb{E}(Y_i^4) < \infty$ it states that $X$ and $Y$ have finite kurtosis. If the assumption of finite fourth moments holds then it is unlikely that statistical inferences using OLS will be dominated by a few observations. This assumption is used to derive the properties of OLS regression statistics in large samples, especially deriving distribution of OLS statistics via large sample approximations.
...
Nevertheless, in many economic research the assumption of finite kurtosis is sensible. Many data have finite range. Consider, for example, that concert, sports, or class attendance is limited by the physical capacities of halls, stadiums, or classrooms, or test scores range from getting them all incorrect to getting them all correct, etc. So, they necessarily have a finite kurtosis. More generally, commonly used distributions, such as normal distribution, have four moments. What is ruled out with this assumption are those distributions that have infinite fourth moments. If distributions have finite fourth moments, then it is unlikely that statistical inferences using OLS will be dominated by a few observations. 

$$\\[0.5in]$$

### <ins>Assumption 5</ins>: No Perfect Multicollinearity {#asmptnpermulticol .unnumbered}
...
This is the key assumption that makes the OLS estimators [unbiased](#ue). This assumption effectively expresses the idea that there may be other factors that impact the dependent variable but these other factors are unrelated to $X_1,\dots,X_n$. What is meant by 'unrelated' here is that given a value of $X_1,\dots,X_n$, the mean distribution of these other factors is $0$, thus the conditional distribution of $u_i$ has a mean of $0$. In other words, this assumption states that the errors are independent of regressors. Also notice that this is in the context of, and conditional on, the particular sample values of X.

So for example, in a randomized controlled experiment lets say subjects are randomly assigned to treatment group $(X=1)$ or the control group $(X=0)$. When the random assignment is done using no information about the subject, $X$ is distributed independently of the subject's personal characteristics, including those that determine Y. Because of this random assignment, the conditional mean of $u$ given $X$ is 0.
<p style="margin-left: 40px">
$\hookrightarrow$
Note that since regression analysis models the conditional mean, $X$ does not need to be distributed independently of all the other factors comprising $u$. It is just that the mean of $u$ cannot be related to $X$; i.e. $\mathbb{E}(u_i | X_i)=0.$
</p>

In observational data $X$ is not randomly assigned in an experiment. Instead, the best the can be hoped for is that $X$ is *as if* randomly assigned, in that $\mathbb{E}(u_i | X_i)=0.$ Whether this assumption holds in a given empirical application with observational data requires careful thought and judgment and we will come back to this issue frequently.

Also notice that the conditional mean assumption $\mathbb{E}(u_i | X_i)=0$ implies that $u_i$ and $X_i$ are uncorrelated, i.e. $Corr(u_i, X_i)=0$ (see the box below).
<p style="margin-left: 40px">
$\hookrightarrow$
...
Another thing to note is that this exogeneity assumption implies that there is no specification bias or specification error in the model. That is, the regression model is correctly specified. Specification error can happen if an important explanatory variable is omitted, or an unnecessary variable is included, or a wrong functional form with respect to the relationship between $Y$ and $X$ variables is chosen. The assumption that the disturbance term has zero expectation implies that this is not the case and the model is correctly specified.

In practice, it can be reasonable to also assume that this exogeneity assumption is satisfied automatically if an intercept is included in the regression equation. This is because the role of the intercept is to pick up any systematic but constant tendency in $Y$ that is not accounted for by the explanatory variables in the regression equation. 

To see this, suppose the regression model is $Y_i = \beta_0 + \beta_1X_i + u_i$, and $\mathbb{E}(u_i|X_i)=\mu_u \neq 0$ for all $i$. Now define a new variable $v_i$ where $v_i = u_i - \mu_u$. We can then rewrite the regression equation as:
\[
\begin{align}
Y_i 
&= \beta_0 + \beta_1X_i + u_i \\
&= \beta_0 + \beta_1X_i + v_i + \mu_u \\
&= \beta_0^* + \beta_1X_i + v_i
\end{align}
\]
where $\beta_0^* = \beta_0 + \mu_u.$ The disturbance term in the re-specified model now satisfies the exogeneity assumption becase
\[
\mathbb{E}(v_i) = \mathbb{E}(u_i - \mu_u) = \mathbb{E}(u_i) - \mathbb{E}(\mu_u) = \mu_u - \mu_u = 0.
\]
The downside of this re-specified model is that the interpretation of the intercept has changed since it absorbed the nonzero component of the error term in addition to whatever had previously been responsible for it. This may not be too problematic, though, when we are not interested in the intercept in the regression model, which is usually the case.
...
> Under the above assumptions the OLS estimator is [**unbiased**](#ue), [**consistent**](#consistency), and has a [**normal sampling distribution**](#clt) in large samples. If these three assumptions hold, and if the sample size is large, then the methods for inference (hypothesis testing using the t-statistic and construction of 95% confidence intervals as $\pm$ 1.96 standard errors) are justified.
>
>
...
This assumption means that $\sigma_u^2$ is also the *unconditional expectation* of $u_i^2$ because 
\[
\begin{align*}
\sigma_u^2=Var(u_i|X_i) 
&= \mathbb{E}\left(u_i^2|X_i\right) - \left[\mathbb{E}(u_i|X_i)\right]^2 \\
&= \mathbb{E}\left(u_i^2|X_i\right)-0 \ \ \ \text{since } \mathbb{E}(u_i|X_i)=\mathbb{E}(u_i)=0 \\
&= \mathbb{E}(u_i^2)=Var(u_i) \ \ \ \text{since $u_i$ and $X_i$ are independent, and } \mathbb{E}(u_i)=0.
\end{align*}
\]

In other words, $\sigma_u^2$ is the *unconditional variance* of $u_i$. Accordingly, $\sigma_u^2$ is often called the *error variance* or *disturbance variance*. Of course $\sigma_u^2$ is unknown and one of the tasks of regression analysis is to estimate the variance of the disturbance term.

p style="margin-left: 40px">
$\hookrightarrow$
Note that here we are thinking about the potential distribution of the disturbance term before the sample is actually generated. Once we have generated the sample, the disturbance term will turn out to be greater in some observations, and smaller in others, but there should not be any reason for it to be more erratic in some observations than in others.
</p>

It is often useful to also write the exogeneity assumption and this homoskedasticity assumption not in terms of the error term but in terms of the conditional mean and conditional variance of $Y_i$:
\[
...
When $Var(u_i|X_i)$ depends on $X_i$, the error term is said to exhibit heteroskedasticity. Also note that heteroskedasticity is present whenever $Var(Y_i|X_i)$ is a function of $X_i$ because $Var(u_i|X_i)=Var(Y_i|X_i).$ If heteroskedasticity is present, the OLS regression coefficients, the $\beta$s, will be inefficient.
...
**Extra:** In matrix notation, if we assume that $u_i$ and $\vec{X}_i^T$ are independent, then the distribution of $u_i$ given $\vec{X}_i^T$ does not depend on $\vec{X}_i^T$. Accordingly, $\mathbb{E}(u_i|\vec{X}_i^T)=\mathbb{E}(u_i)=0$ and $Var(u_i|\vec{X}_i^T)=\sigma_u^2.$ 
