...
In terms of the notation, $Y$ denotes the dependent variable, or regressand, and $X$s denote the explanatory variables, or regressors, with $k^{th}$ regressor indicated by $X_k$. The subscript $i$ is used to indicate cross-sectional data, and $t$ time series data, and they denote the $i^{th}$ or $t^{th}$ observation or value, and $X_{ki}$ or $X_{kt}$ denote the $i^{th}$ or $t_{th}$ observation on variable $X_k$. 
...
\]
Separately, notice that before its decomposition into random and nonrandom components, if we divide the numerator and the denominator of the expression for $\hat{\beta}_1$ by $n-1$, we can also express it as:
\[
\hat{\beta}_1 = \frac{\frac{1}{n-1}\displaystyle\sum_{i=1}^n(X_i-\bar{X})(Y_i-\bar{Y})}{\frac{1}{n-1}\displaystyle\sum_{i=1}^n(X_i-\bar{X})^2} = \frac{\hat{\sigma}_{XY}}{\hat{\sigma}_X^2}.
\]
...
\[
\sigma_{\hat{\beta}_0} = \sigma_u^2 \frac{\displaystyle\sum_{i=1}^n X_i^2}{n\displaystyle\sum_{i=1}^n (X_i-\bar{X})^2} = \sigma_u^2 \left( \frac{1}{n} + \frac{\bar{X}^2}{\displaystyle\sum_{i=1}^n (X_i-\bar{X})^2} \right).
\]
Looking at the expressions for $\sigma_{\hat{\beta}_1}$ and $\sigma_{\hat{\beta}_1}$ we can see that they will be smaller, the larger $\sum(X_i-\bar{X})^2$ is. The size of $\sigma_{\hat{\beta}_1}$ in turn depends both on the number of observations and on the size of the deviations of $X_i$ from its sample mean.
