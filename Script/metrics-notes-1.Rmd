...
<span style="font-size:0.75em;">The discussions in here are based on Hansen, B E (2022) *Probability and Statistics for Economists* Princeton University Press; Stock, J H and Watson M W (2020) *Introduction to Econometrics* (4th Global ed) Pearson; Wooldridge, J M (2020) *Introductory Econometrics: A Modern Approach* (7th ed) Cengage; Hansen, B E (2022) *Econometrics* Princeton University Press.
</span>
...
Consider the linear regression model $Y_i = \beta_0 + \beta_1X_i + u_i,$ and let $b_0$ and $b_1$ be some estimators of $\beta_0$ and $\beta_1$. The value of $Y_i$ is then predicted by the line $b_0+b_1X_i.$
...
\]
Rearranging the terms and dividing by $n$ shows that the OLS estimators $\widehat{\beta}_0$ and $\widehat{\beta}_1$ need to satisfy the following two equations
\[
\begin{align*}
...
&=\left(\bar{Y}-\widehat{\beta}_1\bar{X}\right) + \widehat{\beta}_1X_i \ , \ \text{for} \ i=1,\dots,n \\[4pt]
...
We can next define the *fitted value* as $\widehat{\vec{Y}} = \mathbf{X}\widehat{\vec{\beta}}$. Since $\vec{Y} = \widehat{\vec{Y}} + \widehat{\vec{u}}$, we have
\[
\vec{Y} = \mathbf{X}\widehat{\vec{\beta}}+\widehat{\vec{u}}.
\]
<p style="margin-left: 40px">
$\hookrightarrow$
Sometimes $\widehat{\vec{Y}}$ is called *predicted value* but this is misleading. The fitted value $\widehat{\vec{Y}}$ is a function of the entire sample, including $\vec{Y}$, so it cannot be interpreted as a valid prediction of $\vec{Y}$. It is therefore more accurate to describe $\widehat{\vec{Y}}$ as *fitted* rather than *predicted* value.
</p>


Equivalently, the *residual* vector is:
\[
\widehat{\vec{u}} = \vec{Y} - \mathbf{X}\widehat{\vec{\beta}}.
\]
<p style="margin-left: 40px">
$\hookrightarrow$
Note that there is a distinction between the <ins>error</ins> $\vec{u}$ and <ins>residual</ins> $\widehat{\vec{u}}$. The error $\vec{u}$ is unobservable, while the residual $\widehat{\vec{u}}$ is an estimator.
</p>

Using the residual vector we also have $\mathbf{X}^T\widehat{\vec{u}}=0$ since $\sum_{i=1}^nX_i\widehat{u}_i=0.$
...
Sometimes also referred to as the *coefficient of determination*, the *regression $R^2$* is the ratio of the sample variance of $\vec{Y}$ explained by $\mathbf{X}.$ In other words, $R^2$ is the ratio of the sample variance of $\widehat{\vec{Y}}$ to the sample variance of $\vec{Y}.$


That is,
\[
R^2 = \frac{ESS = \displaystyle\sum_{i=1}^n(\widehat{Y}_i-\bar{Y})^2}{TSS = \displaystyle\sum_{i=1}^n(Y_i-\bar{Y})^2}
\]
where ESS is the abbreviation of explained sum of squares and TSS is of total sum of squares. In matrix notation:
\[
R^2 = \frac{\left(\widehat{\vec{Y}}-\vec{\bar{Y}}\right)^2}{\left(\vec{Y}-\vec{\bar{Y}}\right)^2}.
\]
The $R^2$ can also be expressed in terms of the fraction of variance of $\vec{Y}$ that is *not explained* by $\mathbf{X}:$
\[
R^2 = 1 - \frac{RSS = \displaystyle\sum_{i=1}^n\widehat{u}_i^2 = \displaystyle\sum_{i=1}^n(Y_i - \widehat{Y}_i)}{TSS = \displaystyle\sum_{i=1}^n(Y_i-\bar{Y})^2}
\]
or in matrix notation:
\[
R^2 = 1 - \frac{\widehat{\vec{u}}^2}{\left(\vec{Y}-\vec{\bar{Y}}\right)^2}
\]
Also note that
\[
\begin{align*}
TSS &= ESS + RSS \\
\displaystyle\sum_{i=1}^n(Y_i-\bar{Y})^2 &= \displaystyle\sum_{i=1}^n(\widehat{Y}_i-\bar{Y})^2 + \displaystyle\sum_{i=1}^n\widehat{u}_i^2
\end{align*}
\]
...
\[
SER = s_\widehat{\vec{u}} = \sqrt{s_\widehat{\vec{u}}^2}
\]
where
\[
s_\widehat{\vec{u}}^2 = \frac{1}{n-k-1}\sum_{i=1}^n\widehat{u}_i^2 = \frac{SSR}{n-k-1}
\]
The estimate $s_\widehat{u}$ is an estimate of the standard deviation in the unobservables affecting $\vec{Y}$. That is, it estimates the standard deviation in $\vec{Y}$ after the effect of $\mathbf{X}$ has been taken out. 
...
These assumptions discuss the statistical properties of OLS estimating the parameters in an underlying population model. These statistical models have nothing to do with a particular sample, but rather with the property of estimators when random sampling is done repeatedly.

For the multiple regression model $Y_i = \vec{X}_i^T \ \vec{\beta}+u_i, \ i=1,\dots,n$ there are there are two sets of assumptions we consider here. One set of assumptions are assumptions about the explanatory variables $vec{X}_^T$ and the other set is about the error terms
...
The population regression model, or what is sometimes called the true model, is linear in parameters. That is, the population regression model can bet written as $Y = \beta_0 + \beta_1X_1 + \beta_2X_2 + \dots + \beta_kX_k + u$ where $\beta_0, \beta_1, \beta_2,\dots,\beta_k$ are the unknown parameters (constants), and $u$ is an unobserved random error or disturbance term.


The key feature in this assumption is that the model is linear in the parameters $\beta_0, \beta_1, \beta_2, \dots, \beta_k.$ Notice that this is different than the *linear conditionally unbiased estimator* discussion above which focused on the estimator $\widehat{\beta}_1$. Here, the population model is linear in parameters.

In time series, the population regression model would be expressed as $Y_t = \beta_0 + \beta_1X_{t1} + \beta_2X_{t2} + \dots + \beta_kX_{tk} + u$ where $t$ in $X_{tj}$ denotes the time period and $j$ indicates one of the $k$ regressors. In this sense, we can let $\vec{X}_t^T = (X_{t1}, X_{t2}, \dots, X_{tk})$ denote the set of all independent varaibles in the equation at time $t$. Similarly, $\mathbf{X}$ can be thought of as an array with $n$ rows and $k$ columns, so the $t^{th}$ row of $\mathbf{X}$ is $\vec{X}_t^T$
...
### <ins>Assumption 2</ins>: Random Sampling: $(\vec{X}_i,Y_i)$ are i.i.d. draws from their joint distribution {#asmptnrandsamp .unnumbered}

This assumption states that $(X_{1i},X_{2i},\dots,X_{ki},Y_i) \ , \ i=1,\dots,n$ are independently and identically distributed (i.i.d.) random variables. This assumption holds automatically if the data are collected by simple random sampling. As a result, under random sampling, errors for different cross-sectional observations, $u_i$ and $u_j$ are independent for any two observations $i$ and $j$ (also see [exogeneity](#asmptnexogen) assumption). 
<p style="margin-left: 40px">
$\hookrightarrow$
...
In simple random sampling, $n$ objects are selected from at random from a population. These $n$ observations are denoted as $Y_1,\dots,Y_n.$ Because the members of the population are included in the sample are selected at random, the values of the observations $Y_i,\dots,Y_n$ are random. So the act of random sampling means that $Y_i,\dots,Y_n$ can themselves be treated as random variables. Before sampled, $Y_1,\dots,Y_n$ can take on many possible values; after they are sampled a specific value is recorded for each observation.


Because $Y_1,\dots,Y_n$ are randomly drawn from the same population, the marginal distribution of $Y_i$ is the same for each $i=1,\dots,n.$ When $Y_i$ has the same marginal distribution for $i=1,\dots,n$, then $Y_1, \dots, Y_n$ are said to be *identically distributed*.
...
We can extend this to vectors. The random vectors $\{\vec{Y}_1,\dots,\vec{Y}_n\}$ are i.i.d. if they are mutually independent with identical marginal distributions *F*. A collection of random vectors $\{\vec{Y}_l,\dots,\vec{Y}_n\}$ is a *random sample* from the population $F$ if the $\vec{Y}_i$ are i.i.d. with distribution $F$, i.e. the population distribution.

Sometimes we need to write the equation for a particular observation $i$: for a randomly drawn observation from the population, we have $Y_i = \beta+0 + \beta_1X_{i1} + \beta_2X_{i2} +\dots + \beta_kX{ik} + u.$ For example, we can write a CEO salary equation for a particular CEO $i$ as $log(salary_i) = \beta_0 + \beta_1log(sales_i)+\beta_2ceotenure_i + u_i.$
...
### <ins>Assumption 3</ins>: $\vec{X}_i$ and $u_i$ have nonzero finite fourth moment {#asmptn4thmmnt .unnumbered}
...
### <ins>Assumption 4</ins>: No Perfect Multicollinearity: $\mathbf{X}$ has full column rank {#asmptnpermulticol .unnumbered}
...
So, for example, the model $consumption = \beta_0 + \beta_1income + \beta_2income^2 + u$ does not violate this assumption but the model $log(consumption) = \beta_0 + \beta_1log(income) + \beta_2log(income^2) + u$ does. In the first model $X_2 = income^2$ is not an exact linear function of $X_1=income$. Whereas in the second model, $X_2=log(income^2)=2log(income)$ is an exact linear function of $X_1=log(income)$ since $X_2=2X_1$ in this case.
...
### <ins>Assumption 5</ins>: Exogeneity: Zero cobnditional mean $\mathbb{E}\left(u_i | \vec{X}_i^T\right)=0$ {#asmptnexogen .unnumbered}

This is the key assumption that makes the OLS estimators unbiased. It states that given $\vec{X}_i^T$, the conditional distribution of $u_i$ has a mean of $0$. This assumption expresses the idea that there may be other factors that impact the dependent variable but these other factors are unrelated to $\vec{X}_i^T$. What is meant by 'unrelated' here is that given a value of $\vec{X}_i^T$, the mean distribution of these other factors is $0$. In other words, this assumption states that the errors are independent of regressors.
...
Therefore the conditional mean assumption $\mathbb{E}(u_i | \vec{X}_i^T)=0$ implies that $u_i$ and $\vec{X}_i^T$ are uncorrelated, i.e. $Corr\left(u_i, \vec{X}_i^T\right)=0$.
<p style="margin-left: 40px">
$\hookrightarrow$
Note that the reverse is not necessarily true. Since correlation is a measure of *linear* association, the conditional mean of $u_i$ given $\vec{X}_i^T$ might be nonzero even if $u_i$ and $\vec{X}_i$ are uncorrelated.
</p>
<p style="margin-left: 40px">
$\hookrightarrow$
However, if $u_i$ and $\vec{X}_i^T$ are correlated, then it must be that $\mathbb{E}(u_i|\vec{X_i}^T) \neq 0.$
</p>
<p style="margin-left: 40px">
$\hookrightarrow$
...
It is therefore common to discuss the conditional mean assumption in terms of possible correlation between $u_i$ and $\vec{X}_i^T$. If they are correlated, then the conditional mean assumption is violated.


In observational data, $\vec{X}$ is not necessarily randomly assigned but we hope that it behaves as if it was randomly assigned, meaning $\mathbb{E}(u_i | \vec{X}_i^T)=0$. Whether this holds requires careful thought and judgment and we will come back to this issue frequently.
...
### <ins>Assumption 6</ins>: Homoskedasticity: Constant error variance $Var\left(u_i|\vec{X}_i^T\right)=\sigma_u^2$ {#asmptnskedasticity .unnumbered}

This assumption states that the variance in the error term, conditional on the explanatory variables, is the *same* for all combinations of outcomes in the explanatory variables. That is, the error term $u_i$ is *homoskedastic* if the variance of the conditional distribution of $u_i$ given $\vec{X}_i^T$ is constant for $i=1,\dots,n$, and it does not depend on $\vec{X}_i^T$. Otherwise the error term is *heteroskedastic*.


The homoskedasticity assumption is distinct from the zero conditional mean assumption $\mathbb{E}(u_i | \vec{X}_i^T)=0$ [discussed above](#exogeneity). That assumption involves the *expected value* of $u_i$ conditional on $\vec{X}_i$, whereas this assumption concerns the *variance* of $u_i$ conditional on $\vec{X}_i^T.$
...
\[
\begin{align*}
\sigma_u^2=Var(u_i|\vec{X}_i^T) 
&= \mathbb{E}\left(u_i^2|{\vec{X}_i^T}^2\right) - \left(\mathbb{E}(u_i|\vec{X}_i^T)\right)^2 \\
&= \mathbb{E}\left(u_i^2|{\vec{X}_i^T}^2\right)-0 \ \ \ \text{since } \mathbb{E}(u_i|\vec{X}_i^T)=\mathbb{E}(u_i)=0 \\
&= \mathbb{E}(u_i^2)=Var(u_i) \ \ \ \text{since $u_i$ and $\vec{X}_i$ are independent, and } \mathbb{E}(u_i)=0.
\end{align*}
\]
...
\[
\mathbb{E}(Y_i|\vec{X}_i^T)=\vec{X}_i^T\vec{\beta} \\
Var(Y_i|\vec{X}_i^T) = \sigma_u^2.
\]
That is, the conditional expectation of $Y_i$ given $\vec{X}_i^T$ is linear in $\vec{X}_i^T$, but the variance of $Y_i$ given $\vec{X}_i^T$ is constant.


A point to note is that the nature of skedasticity concerns with the conditional variance and not the unconditional variance. By definition, the unconditional variance $\sigma_u^2$ is a constant and is independent of the regressors $\vec{X}_i^T$. So in discussing the variance as a function of the regressors, it refers to conditional variance.


When $Var(u_i|\vec{X}_i^T)$ depends on $\vec{X}_i^T$, the error term is said to exhibit heteroskedasticity. Also note that heteroskedasticity is present whenever $Var(Y_i|\vec{X}_i^T)$ is a function of $\vec{X}_i^T$ because $Var(u_i|\vec{X}_i^T)=Var(Y_i|\vec{X}_i^T).$
...
### <ins>Assumption 7</ins>: Normal errors: Conditional distribution of $u_i$ given $\vec{X}_i$ is normal {#asmptnnormerr.unnumbered}

The distribution of $(u_i|\vec{X}_i^T)$ has a mean $0$ by virtue of [exogeneity assumption](#asmptnexogen), and variance of $Var(u_i|\vec{X}_i^T)$. That is, $(u_i|\vec{X}_i^T) \sim N\left(0, Var(u_i|\vec{X}_i)\right).$ 


Since the discussion in [homoskedasticity assumption](#asmptnskedasticity) states that $Var(u_i|\vec{X}_i^T)=\sigma_u^2$, the conditional distribution becomes: $(u_i|\vec{X}_i^T) \sim N(0, \sigma_u^2).$ 


Also, $u_i$ and $\vec{X}_i^T$ are independently distributed since the conditional distribution of $(u_i|\vec{X}_i^T)$ does not depend on $\vec{X}_i^T$. Additionally, due to [random sampling assumption](#asmptnandsamp), $u_i$ is distributed independently of $u_j$ for all $j\neq i$. Therefore, $u_i$ and $\vec{X}_i^T$ are independently distributed and $u_i \sim i.i.d. N(0,\sigma_u^2).$  


This assumption underpins obtaining the exact sampling distributions of the OLS estimators that are conditional on the regressors in the sample. Specifically, the OLS estimators have normal sampling distributions (see [unbisased estimator](#ue) discussion below) which leads to $t$ and $F$ distributions for $t$ statistics and $F$ statistics. If the error is not normally distributed, the distribution of a $t$ statistic is not exactly $t$, and an $F$ statistic does not have an exact $F$ distribution for any sample size. This assumption can be dropped if the sample size is reasonably large. (See [OLS Asymptotics](#olsasymptot) discussion.)



$$\\[0.5in]$$

### <ins>Assumption 8</ins>: No Serial Correlation $\mathbb{E}\left(u_i,u_j | \mathbf{X}\right)$ (especially important in time series) {#assmptnnosercorr .unnumbered}

This assumption is already a natural consequence of random sampling whereby $u_i$ and $u_j$ are independent for any two observations $i$ and $j$. Under random sampling, the errors for different observations are independent conditional on the explanatory variables in the sample. Therefore, serial correlation is a potential problem mainly for regressions with time series data.


This assumption states that conditional on $\mathbf{X}$, the errors in two different time periods are not correlated: $Corr(u_i,u_j | \mathbf{X})=0$, for all $i\neq j$. If we treat $\mathbf{X}$ as nonrandom, then we can ignore the conditioning on $mathbf{X}$ and the assumption becomes $Corr(u_i,u_j) = 0$ for all $i\neq j$. When considering whether this assumption holds, it is common to focus on this unconditioned version of the assumption because of its simple interpretation.
...
Suppose we are evaluating an estimator many times. We are doing this evaluation over repeated randomly drawn samples. We would hope that, on average, we would get the right answer. So a reasonable property of an estimator is that the mean of its sampling distribution equals the population mean, $\mu_Y$. If that is the case, then the estimator is unbiased. That is, the estimator $\widehat{\mu}_X$ is unbiased if $\mathbb{E}(\widehat{\mu}_x) = \mu_X$, where $\mathbb{E}(\widehat{\mu}_X)$ is the mean of the sampling distribution of $\widehat{\mu}_X.$ Otherwise, $\widehat{\mu}_X$ is biased.
...
It is also possible for an estimator to be unbiased for a set of distributions while being biased for other sets. For example, the estimator $\widetilde{\mu}_X=0$ is biased when $\mu_X\neq 0$ and unbiased when $\mu_X=0$. This may be a trivial example but it is useful to be clear about the contexts where an estimator is unbiased or biased. 
...
These ideas carry over to the OLS estimators. Since the OLS estimators are calculated using a random sample, $\widehat{\beta}_0$ and $\widehat{\beta}_1$ are themselves random variables that take on different values from one sample to the next. The probability of these different values is summarized in their sampling distributions.


We obtained the expressions for $\widehat{\beta}_0$ and $\widehat{\beta}_1$ in the [OLS estimator in single regression](#ols-single) discussion. Lets replicate them here for convenience:
\[
\widehat{\beta}_0=\bar{Y}-\widehat{\beta}_1\bar{X} \ \ \ \ \ , \ \ \ \ \ \widehat{\beta}_1 = \frac{\displaystyle\sum_{i=1}^n(X_i-\bar{X})(Y_i-\bar{Y})}{\displaystyle\sum_{i=1}^n(X_i-\bar{X})^2} 
\]
Let's now rewrite the expression for $\widehat{\beta}_1$ in terms of the regressors and errors. This will be useful in demonstrating the unbiasedness of the OLS estimators. First note that since $Y_i = \beta_0 + \beta_1X_i + u_i$ for a linear regression with one regressor, we have $(Y_i-\bar{Y}) = \beta_1(X_i - \bar{X}) + u_i - \bar{u}$. We can plug this in to the numerator of the expression for $\widehat{\beta}_1:$
\[
\begin{align*}
...
\end{align*}
\]
We can now rewrite $\widehat{\beta}_1$ in terms of the regressors and errors with this expression in the numerator:
\[
\widehat{\beta}_1 = \frac{\beta_1\displaystyle\sum_{i=1}^n(X_i-\bar{X})^2 + \sum_{i=1}^n(X_i-\bar{X})u_i}{\displaystyle\sum_{i=1}^n(X_i-\bar{X})^2} = \beta_1 + \frac{\displaystyle\sum_{i=1}^n(X_i-\bar{X})u_i}{\displaystyle\sum_{i=1}^n(X_i-\bar{X})^2}
\]

We can now use this expression to derive the expectation of $\widehat{\beta_1}$:
\[
\begin{align*}
\mathbb{E}\left(\widehat{\beta}_1 | X_1,...,X_n\right) 
...
\end{align*}
\]
Notice that $\widehat{\beta}_1$ is unbiased after averaging over all samples $X_1,\dots,X_n$ because it is unbiased given $X_1,\dots,X_n.$ Therefore, the unbiasedness of $\widehat{\beta}_1$ follows from the above derivation and from the law of iterated expectations:
\[
\mathbb{E}(\widehat{\beta}_1) = \mathbb{E}\left(\mathbb{E}(\widehat{\beta}_1 | X_1,\dots,X_n)\right) = \beta_1.
\]
Similarly, we can show the unbiasedness of $\widehat{\beta}_0$ as follows:
\[
\begin{align*}
\mathbb{E}(\widehat{\beta}_0) 
&= \mathbb{E}(\bar{Y} - \widehat{\beta}_1\bar{X}) \\[4pt]
&= \frac{1}{n}\sum_{i=1}^n\mathbb{E}(Y_i)-\mathbb{E}(\widehat{\beta}_1)\bar{X} \\[4pt]
&= \frac{1}{n}\sum_{i=1}^n(\beta_0+\beta_1X_i) - \beta_1\bar{X} \\[4pt]
&= \frac{1}{n}(n\beta_0 + n\beta_1\bar{X})-\beta_1\bar{X} \\[4pt]
&= \beta_0.
\end{align*}
\]
Thus, $\widehat{\beta}_0$ is an unbiased estimator of $\beta_0.$

<div style="background-color:rgba(0, 0, 0, 0.0470588); text-align:left; vertical-align: middle; padding:0px; margin-top:10px; margin-bottom:20px">
>This discussion shows that the central tendency of $\widehat{\beta}_j$ for $j=0,\dots,k$ is the value of population parameter $\beta_j$ for any $j$. That is, the OLS estimators are unbiased estimators of the population parameters.
>
>
We can also have a measure of the spread in the sampling distribution of $\widehat{\beta}_j$
\[
Var(\widehat{\beta}_j) = \frac{\sigma_u^2}{TSS_j(1-R_j^2)} = \frac{\sigma_u^2}{\displaystyle\sum_{i=1}^n\widehat{u}_{ij}^2}
\]
Therefore,

\[
\widehat{\beta}_j \sim N\left(\beta_j \ , \ \frac{\sigma_u^2}{TSS_j(1-R_j^2)}\right)
\]
...
The class of linear conditionally unbiased estimators consists of all estimators of $\beta_1$ that are both linear functions of $Y_1,\dots,Y_n$ and that are unbiased conditional on $X_1,\dots,X_n.$ So, if $\widehat{\beta}_1$ is a linear estimator then it can be written as
\[
\widehat{\beta}_1 = \sum_{i=1}^nw_iY_i
\]
where the weights $w_1,\dots,w_n$ can depend on $X_1,\dots,X_n$ but not on $Y_1,\dots,Y_n.$

The estimator $\widehat{{\beta}}_1$ is conditionally unbiased if $\mathbb{E}(\widehat{\beta}_1 | X_1,\dots,X_n) = \beta_1$ as discussed above in Section 2.1. So we just need to show that it is linear. For this, we again start with the expression for $\widehat{\beta}_1$ we obtained in Section 1 and reproduced in Section 2.1:
\[
\begin{align*}
\widehat{\beta}_1 
...
&= \sum_{i=1}^n\widehat{w}_iY_i \ \ \ \ \text{where } \widehat{w}_i=\frac{(X_i-\bar{X})}{\displaystyle\sum_{j=1}^n(X_j-\bar{X})^2}.
\end{align*}
\]
Since the weights $\widehat{w}_i, \ i=1,\dots,n$ in this expression for $\widehat{\beta}_1$ depend on $X_1,\dots,X_n$ but not on $Y_1,\dots,Y_n$, the OLS estimator $\widehat{\beta}_1$ is a linear estimator. Since it is also conditionally unbiased, it is linear conditionally unbiased estimator.
...
MSE is a standard measure of accuracy. If $\widehat{\theta}$ is an estimator of $\theta$, then
\[
MSE(\widehat{\theta})=\mathbb{E}\left((\widehat{\theta}-\theta)^2\right)
\]
By expanding the square we find that
\[
\begin{align*}
MSE(\widehat{\theta})=\mathbb{E}\left((\widehat{\theta}-\theta)^2\right)
&= \mathbb{E}\left(\left(\widehat{\theta}-\mathbb{E}(\widehat{\theta})+\mathbb{E}(\widehat{\theta})-\theta\right)^2\right) \\
&= \mathbb{E}\left(\left(\widehat{\theta}-\mathbb{E}(\widehat{\theta})\right)^2\right) +
2 \mathbb{E}\left(\widehat{\theta}-\mathbb{E}(\widehat{\theta})\right)  \left(\mathbb{E}(\widehat{\theta})-\theta\right) + 
\left(\mathbb{E}(\widehat{\theta})-\theta\right)^2 \\
&= Var(\widehat{\theta})+\left(bias(\widehat{\theta})\right)^2.
\end{align*}
\]
...
\[
\widetilde{X} = \frac{1}{n}\left(\frac{1}{2}X_1 + \frac{3}{2}X_2 + \frac{1}{2}X_3 + \frac{3}{2}X_4 + \dots + \frac{1}{2}X_{n-1} + \frac{3}{2}X_n \right).
\]
The mean of $\widetilde{X}$ is $\mu_X$ so it is unbiased. Its variance is
\[
Var(\widetilde{X}) = \left(\frac{0.5^2 + 1.5^2}{2}\right)\frac{\sigma^2}{n} = 1.25\frac{\sigma^2}{n}.
\]
This shows that $\widetilde{X}$ has 1.25 times higher variance than $\bar{X}$, so $\bar{X}$ is more efficient than $\widetilde{X}.$


The estimators $\bar{X},X_1,$ and $\widetilde{X}$ all have a common structure: they are weighted averages of $X_1,\dots,X_n.$ We have seen that both $X_1$ and $\widetilde{X}$ have higher variances than $\bar{X}.$ These reflect a more general result: 
...
> 
This class of estimators is $\widetilde{\mu}=\sum_{i=1}^nw_i\mathbb{E}(X_i)$ where the weights $w_i$ are freely selected. Unbiasedness requires
\[
\mathbb{E}(\widetilde{\mu})=\mathbb{E}\left(\sum_{i=1}^nw_iX_i\right) = \sum_{i=1}^nw_i\mathbb{E}(X_i) = \sum_{i=1}^nw_i\mu = \mu
\]
which holds if and only if $\sum_{i=1}^nw_i=1.$ The variance of $\widehat{\mu}$ is
\[
Var(\widetilde{\mu}) = Var\left(\sum_{i=1}^nw_iX_i\right) = \sigma^2\sum_{i=1}^nw_i^2.
\]
Hence, the best, i.e. minimum variance, estimator is $\widetilde{\mu}=\sum_{i=1}^nw_i\mathbb{E}(X_i)$ with the weights selected to minimize $\sum_{i=1}^nw_i^2$ subject to the restriction $\sum_{i=1}^nw_i=1.$
>
>
...
Gauss-Markov Theorem says that the OLS estimators $\widehat{\beta}_0$ and $\widehat{\beta}_1$ are efficient among all estimators that satisfies the following set of specific assumptions:
...
>
To demonstrate why OLS are BLUE, we start by recalling from [linear conditionally unbiased estimators](#lue) discussion the expression of a linear estimator is $\widetilde{\beta}_1 = \sum_{i=1}^nw_iY_i.$ We then substitute this into $Y_i=\beta_0+\beta_1X_i+u_i:$
\[\begin{align*}
\widetilde{\beta}_1 
...
\[
\begin{align*}
\mathbb{E}(\widetilde{\beta}_1 | \vec{X}^T = X_1,\dots,X_n) 
...
\]
>
In the [unbiased estimator](#ue) discussion we established the conditional unbiasedness of $\widehat{\beta}_1.$ Because $\widetilde{\beta}_1$ is conditionally unbiased by assumption - that is $\mathbb{E}(\widetilde{\beta}_1 | X_1,\dots,X_n) = \beta_1$ - we therefore have
\[
\mathbb{E}(\widetilde{\beta}_1 | \vec{X}^T = X_1,\dots,X_n) = \beta_0\sum_{i=1}^nw_i + \beta_1\sum_{i=1}^nw_iX_i = \beta_1
\]
...
Thus,
\[
Var(\widehat{\beta}_1 | X_1,\dots,X_n) = Var(\sum_{i=1}^nw_iu_i | X_1,\dots,X_n) = \sum_{i=1}^n\sum_{j=1}^nw_iw_jCov(u_iu_j|X_1,\dots,X_n)
\]
...
\[
Var(\widehat{\beta}_1 | X_1,\dots,X_n) = \sigma_u^2\sum_{i=1}^nw_i^2.
\]
Note here that $\widehat{\beta}_1$ is a case where the weights are $w_i = \widehat{w}_i = \frac{(X_i-\bar{X})}{\sum_{j=1}^n(X_j-\bar{X})^2}$ as discussed under [linear conditionally unbiased estimators](#lue). So we just have to show that any other weights would yield a larger variance than the weights of $\widehat{\beta}_1.$
>
For that, let $w_i=\widehat{w}_i + d_i$ so that
\[
\begin{align*}
\sum_{i=1}^nw_i^2 
&= \sum_{i=1}^n(\widehat{w}_i+d_i)^2 \\[6pt]
&= \sum_{i=1}^n\widehat{w}_i^2 + 2\sum_{i=1}^n\widehat{w}_id_i + \sum_{i=1}^nd_i^2 \\[6pt]
&= \sum_{i=1}^n\widehat{w}_i^2 + 2\left(\frac{\displaystyle\sum_{i=1}^n(X_i-\bar{X})d_i}{\displaystyle\sum_{j=1}^n(X_j-\bar{X})^2}\right) + \sum_{i=1}^nd_i^2 \\[6pt]
&= \sum_{i=1}^n\widehat{w}_i^2 + 2\left(\frac{\displaystyle\sum_{i=1}^nd_iX_i-\bar{X}\displaystyle\sum_{i=1}^nd_i}{\displaystyle\sum_{j=1}^n(X_j-\bar{X})^2}\right) + \sum_{i=1}^nd_i^2 \\[6pt]
&= \sum_{i=1}^n\widehat{w}_i^2 + 2\left(\frac{\left(\displaystyle\sum_{i=1}^nw_iX_i - \displaystyle\sum_{i=1}^n\widehat{w}_iX_i\right)-\bar{X}\left(\displaystyle\sum_{i=1}^nw_i -\displaystyle\sum_{i=1}^n\widehat{w}_i\right)}{\displaystyle\sum_{j=1}^n(X_j-\bar{X})^2}\right) + \sum_{i=1}^nd_i^2 \ \ \ \ \text{since } d_i=w_i-\widehat{w}_i \\[6pt]
&= \sum_{i=1}^n\widehat{w}_i^2 + 0 + \sum_{i=1}^nd_i^2 \ \ \ \ \text{because of two restrictions } \sum_{i=1}^nw_i=0 \text{ and } \sum_{i=1}^nw_iX_i = 1.
\end{align*}
\]
Thus,
\[
\sigma_u^2\sum_{i=1}^nw_i^2 = \sigma_u^2\sum_{i=1}^n\widehat{w}_i^2 + \sigma_u^2+\sum_{i=1}^nd_i^2.
\]
If we then substitute this back into the conditional variance expression
\[
Var(\widetilde{\beta}_1 | X_1,\dots, X_n) - Var(\widehat{\beta}_1 | X_1,\dots,X_n) = \sigma_u^2\sum_{i=1}^nd_i^2.
\]
Therefore, for any $i=1,\dots,n$, $\widetilde{\beta}_1$ has a greater conditional variance than $\widehat{\beta}_1$ unless $d_1=0$. If $d_i=0$ for all $i$, on the other hand, then $w_i=\widehat{w}_i$ and $\widetilde{\beta}_1 = \widehat{\beta}_1$, which proves that OLS is BLUE. 
...
\]
These ideas carry over to the OLS estimators $\widehat{\beta}_j$.

$$\\[0.5in]$$

## Consistency of OLS

The [Unbiased Estimator](#ue) section established that $\widehat{beta}_0$ and $\widehat{\beta}_1$ are unbiased estimators. The same set of assumptions - Assumptions 1 through 5 - also implies consistency of OLS. Recall that in the [unbiased estimator](#ue) section we expressed $\widehat{\beta}_1 as
\[
\begin{align*}
\widehat{\beta}_1 
...
\[\begin{align*}
plim \ \widehat{\beta}_1 
&= \beta_1 + \frac{Cov(X_1,u)}{Var(X_1)} \\
&= \beta_1 \ \ \ \text{because } Var(X_1) > 0 \text{ and } \mathbb{E}(u|X_1)=0 \text{ implies } Cov(X_1,u) = 0
\end{align*}
\]
In general, $\widehat{\beta}_j$ has a probability distribution that represents its possible values in different size $n$ samples. Since $\widehat{\beta}_j$ is unbiased, this distribution has a mean value of $\beta_j$. If this estimator is consistent, then the distribution of $\widehat{\beta}_j$ becomes more and more tightly distributed around $\beta_j$ as the sample size grows. As $n$ tends to infinity, the distribution of $\widehat{\beta}_j$ collapses to a single point $\beta_j$.
...
In simple regression case, the *inconsistency*, or as sometimes also called *asymptotic bias*, in $\widehat{\beta}_1$ is
\[
plim \ \widehat{\beta}_1 - \beta_1 = \frac{Cov(X_1,u)}{Var(X_1)}.
\]
Since $Var(X_1) > 0$, the inconsistency in $\widehat{\beta}_1$ is positive if $X_1$ and $u$ are positively correlated, and negative if negatively correlated. If the covariance between $X_1$ and $u$ is small relative to $Var(X_1)$, the inconsistency can be negligible. However, we cannot estimate how big $Cov(X_1,u)$ is because $u$ is unobserved.


We can nevertheless use this inconsistency approach to derive, for example, the asymptotic version of the omitted variable bias. Suppose the true model $Y=\beta_0+\beta_1X_1+\beta_2X_2+v$ satisfies the first five Gauss-Markov assumptions. Then $v$ has a $0$ mean and is uncorrelated with $X_1$ and $X_2$. From law of large numbers, we also know that $\widehat{\beta}_0,\widehat{\beta}_1,$ and $\widehat{\beta}_2$ are consistent. If we omit $X_2$ from the regression and carry out a simple regression of $Y$ on $X_1$ then $u=\beta_2X_2+v.$ If we let $\widetilde{\beta}_1$ denote the simple regression slope estimator, then
\[
plim \ \widetilde{\beta}_1 = \beta_1 + \beta_2\left(\frac{Cov(X_1,X_2)}{Var(X_1)}\right).
\]
...
If $X_1$ and $X_2$ are uncorrelated in the population, then $Cov(X_1,X_2)/Var(X_1)=0$ and $\widetilde{\beta}_1 is a consistent estimator of $\beta_1$.
<p style="margin-left: 40px">
$\hookrightarrow$
*Note*: This does not necessarily mean $\widetilde{\beta}_1$ is unbiased.
</p>
